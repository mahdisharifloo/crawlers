...
class CommentSpider(scrapy.Spider):
    ...
    def parse(self, response):#NOCHANGE
        for href in response.css('.c-navi-new-list__sublist-option--item a'): #big categroies like phone, etc NOCHANGE
            yield response.follow(href, self.parse_categories)

    def parse_categories(self, response):#NOCHANGE
        for href in response.css('div.js-product-box a'): #each product in listing page NOCHANGE
            yield response.follow(href, self.parse_product)

        # follow pagination links
        for url in response.css('a.c-pager__item::attr(href)').extract(): # page number links NOCHANGE
    ...
    def parse_product(self, response):
        ...
        def extract_with_xpath(query):
            res = response.xpath(query).extract_first()
            return res.strip() if res is not None else None
        res = re.search('(.*)dkp-(.*)/(.*)', response.url)
        id = res.group(2) #unique for each product it's side of dkp in url
        comments_url = 'https://www.digikala.com/ajax/product/comments/{}/?mode=buyers'.format(id) # there is another page that comments exist

        attrs = []
        for attribute_part in response.xpath('//ul[@class="c-params__list"]/li'):
            attr_key_part = attribute_part.xpath('//div[@class="c-params__list-key"]//span//text()').extract()
            attr_value_part = attribute_part.xpath('//div[@class="c-params__list-value"]//span//text()').extract()
            for attr_key, attr_value in zip(attr_key_part, attr_value_part):
                if {attr_key.strip():attr_value.strip()} not in attrs:
                    attrs.append({attr_key.strip(): attr_value.strip()})

        comment_request = response.follow(comments_url, self.parse_comments)

        comment_request.meta['product_id'] = id
        comment_request.meta['attrs'] = attrs
        comment_request.meta['title'] = extract_with_css('.c-product__title::text')
        comment_request.meta['price'] = extract_with_xpath('//div[@class="c-price__value js-price-value"]/text()')
    ...
    def parse_comments(self, response):
        ...
        for href in response.css('.c-pager__item'):
            ...
            comment_request.meta['price'] = response.meta.get('price', None)
            comment_request.meta['category'] = response.meta.get('category', None)
            comment_request.meta['rate'] = product_rate
            comment_request.meta['attrs'] = response.meta.get('attrs', None)
...
## c-product__seller-price-raw js-price-value  # for extract the image paths